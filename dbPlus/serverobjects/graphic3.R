# This is the code which produces graphic3
...
# Function of x - country name, y - region name, and a - type of mobility.
graphic3 <- function(x, y, a) {
  
  s <-
    filter(mobdata, country_region == x, sub_region_1 == y) %>% 
    rename(replace = 
...
# for a specified country, and then a specified region, 5 random sub-regions which exist within
# the region will have their daily mobility plotted

# this graphic will only be plotted for countries with sub-region level data
# this graphic should be more useful than 